...
export const users: User[] = [
  ...
  },
  {
    ...
    fleetId: 'fleet-acme',
    roles: ['driver'],
    driverId: 'driver-1', // MUST match id for drivers
  ...
  },
  {
    ...
    fleetId: 'fleet-acme',
    roles: ['driver'],
    driverId: 'driver-2', // MUST match id for drivers
    ...
    stops: [
      {
        ...
        state: 'AZ',
        zip: '85001',
        scheduledDate: pickupDate - 86400000,
        scheduledTime: '08:00',
        isCompleted: false,
      },
      {
        ...
        state: 'NV',
        zip: '89101',
        scheduledDate: deliveryDate - 86400000,
        scheduledTime: '16:00',
        isCompleted: false,
      },
    ],
...
export function validateReferentialIntegrity(): void {
  ...
  // Check: User with role='driver' has matching driverId
  users.forEach((user) => {
    if (user.roles.includes('driver')) {
      if (!user.driverId) {
        errors.push(`User ${user.id}: role=driver but driverId missing`)
      } else if (user.driverId !== user.id) {
        errors.push(`User ${user.id}: driverId ${user.driverId} does not match uid`)
      }
    }
  })

  // Check: Unique IDs per collection
  const checkUnique = (collection: any[], name: string) => {
